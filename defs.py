...
def reply(update, context, force_reply=True):
    context.bot.send_message(
        chat_id=update.effective_chat.id, 
        text='Estou marcando a sua mensagem?'
        )
...
def fake(update, context):
    context.bot.send_photo(
        chat_id=update.message.chat_id,
        photo=open("c:/Users/Luiz/Desktop/bot/fake.jpg", "rb")
    )

def jojo(update, context):
    context.bot.send_photo(
        chat_id=update.message.chat_id,
        photo=open("c:/Users/Luiz/Desktop/bot/Jojo.jpg", "rb")
    )

def gorfe(update, context):
    context.bot.send_photo(
        chat_id=update.message.chat_id,
        photo=open("c:/Users/Luiz/Desktop/bot/bolinadegorfe.jpg", "rb")
    )


def pinto(update, context):
    context.bot.send_photo(
        chat_id=update.message.chat_id,
        photo=open("c:/Users/Luiz/Desktop/bot/pinto.jpg", "rb")
    )

def bola(update, context):
    context.bot.send_photo(
        chat_id=update.message.chat_id,
        photo=open("c:/Users/Luiz/Desktop/bot/bola.jpg", "rb")
    )

def xd(update, context):
    context.bot.send_photo(
        chat_id=update.message.chat_id,
        photo=open("c:/Users/Luiz/Desktop/bot/xd.jpg", "rb")
    )
...
/rule34
/trap
/pinto = Tente sua sorte e veja no que dá
/xd
'''
        )
...
def batman(update, context):
    context.bot.send_photo(
        chat_id=update.effective_chat.id,
        photo='https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcRZau-OwZDJif7dr4NnDLJjLTTGmbQa1yZT1g&usqp=CAU'
    )
    context.bot.send_message(
        chat_id=update.effective_chat.id,
        text="O dever me chama!"
    )
...
def trap(update, context):
    ...
    context.bot.send_photo(
        chat_id=update.message.chat_id,
        photo=open("c:/Users/Luiz/Desktop/bot/trap/Imagem (" + str(n) + ").jpg", "rb")
    )
...
def mamaco(update, context, output_path="c:/Program Files/Python38/"):
    import pytube
    link = update.message.text[8:]
    nome = pytube.YouTube(link).title
    print(nome)
    pytube.YouTube(link).streams.first().download(filename=nome)
    context.bot.send_video(
        chat_id=update.effective_chat.id,
        video=open(f"c:/Program Files/Python38/{nome}.mp4", 'rb')
    )
...
def previsão(update, context):
    import requests
    import json
    from translate import Translator
    city = update.message.text[7:]
    requisicao = requests.get(f"http://pro.openweathermap.org/data/2.5/forecast/hourly?q={city.lower()}&appid=b1a41ac5ed47d74bc1274e5159e21453&lang=pt-br")
    cotacao = requisicao.json()
    b = cotacao
    a = ("Descrição: " + b["weather"][0]["description"])
    translator= Translator(to_lang='pt-br')
    translation = translator.translate(a)
    context.bot.send_message(
        chat_id=update.effective_chat.id,
